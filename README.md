...
Besides checking `http://`, `https://` and `gemini://` URLs, you can check for:
  ...
  * DNS changes (useful if you’re maintaining a hosts file somewhere due to unreliable DNS or an awkward proxy situation): URLs starting `dns://` will return a list of all current IPs, each enclosed in parentheses. So for example to be alerted if `131.111.179.82` ceases to be one of the IP addresses of `srcf.net`, use `dns://srcf.net (131.111.179.82)`
